...
//				R.drawable.shade_bg));
		bgShadeContent.setBackgroundResource(R.color.bg);
			...
			if (mScroller.computeScrollOffset()) {
				...
				if (oldX != x || oldY != y) {
					if (mSlidingView != null) {
						...
						if (x < 0)
							bgShade.scrollTo(x + 20, y);// 背景右移
						else
							bgShade.scrollTo(x - 20, y);// 背景左移
					}
				}
				invalidate();
			}
...
//		Log.i("getMenuViewWidth", mMenuView.getWidth()+"===width");
